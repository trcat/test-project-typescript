...
import API from '@/rest/add-test';
import { QuestionType, QuestionData, CreateTestResultData } from '@/data';
...
export default class AddTest extends Vue {
    ...
    private questions: QuestionEditData[] = [];
    private dialogTitle = '';
    private dialogForm: QuestionEditData = {
        id: '',
        content: '',
    ...
    private saveTest(formName: string) {
        (this.$refs[formName] as any).validate((valid: any) => {
            if (valid) {
                ...
                const callback = (r: CreateTestResultData) => {
                    this.$message({
                        message: r.message,
    ...
    private editQuestion(questionData: QuestionEditData, index: number) {
        this.dialogTitle = `编辑试题 ${index}`;
        this.dialogForm = questionData;
        this.showQuestionDialog = true;
    }
    private deleteQuestion(questionData: QuestionEditData) {
        const result: QuestionEditData[] = [];
    ...
    private saveQuestion(formName: string) {
        (this.$refs[formName] as any).validate((valid: any) => {
            if (valid) {
                let stop = false;
                for (const o in this.dialogForm.options) {
    ...
    private configAnswer(buttonName: string) {
        const options = this.dialogForm.options;
    ...
    private resetOptionButtons() {
        for (const o in this.dialogForm.options) {
            if (this.dialogForm.options.hasOwnProperty(o)) {
